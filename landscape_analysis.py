...
artifact_counters: Dict[str, int] = {
    "generate_filter_plan_from_rule": 0,
    "generate_filter_function_from_plan": 0,
}
...
def llm_parser(*args, **kwargs):
    raise RuntimeError("llm_parser() is not defined in preserved functions.")

def generate_landscape_table(*args, **kwargs):
    raise RuntimeError("generate_landscape_table() is not defined in preserved functions.")
    ...
    for trial_id in nct_ids:
        ...
        try:
            ...
            row = {
                ...
                "results_posted": _os_pfs_only(results),
            }
            if row["NCTId"]:
                all_rows.append(row)
        except Exception as e:
            print(f"Error fetching data for NCTId {trial_id}: {e}")
...
def main():
    print("=== TrialsEcho Landscape Pipeline ===")
    # 1) OpenAI API key
    key = load_api_key()
    ...
    _write_csv_artifact("original_df_full.csv", df)
    _write_csv_artifact("df_prefiltered.csv", df_prefiltered)
    _write_csv_artifact("df_filtered.csv", df_filtered)
    _write_csv_artifact("failure_df.csv", failure_df)
    ...
    _write_csv_artifact("landscape_table.csv", landscape_table)
...
# === Preserved definitions (functions/classes only) ================
...
"""
    ...
    _write_json_artifact(
        os.path.join("intermediate", "01_generate_landscape_selection_rules.json"),
        parsed_rules,
    )
...
Respond only with the JSON object. Make sure do not include any markdown or "json" at front. Your output should begin with { and end with }.
    ...
    _write_json_artifact(
        os.path.join("intermediate", "02_infer_retriever_from_rules.json"),
        parsed,
    )
    return parsed
...
"""
    ...
    _write_json_artifact(
        os.path.join(
            "intermediate",
            f"03_generate_filter_plan_from_rule_{idx:02d}_{filter_name}.json",
        ),
        {
...
def build_landscape_plans_from_rules(
    ...
    _write_json_artifact(
        os.path.join("intermediate", "04_build_landscape_plans_from_rules.json"),
        plans,
    )
    return plans
...
def generate_filter_function_from_plan(plan: dict) -> str:
    ...
    code = "\n".join(lines)
    idx = _next_artifact_index("generate_filter_function_from_plan")
    filter_name = _sanitize_filename(plan.get("filter_name", f"generated_filter_{idx:02d}"))
    _write_text_artifact(
        os.path.join(
            "generated_filters",
            f"05_generate_filter_function_from_plan_{idx:02d}_{filter_name}.py",
        ),
        code,
    )
    return code
