def targeted_or_immunotherapy_inclusion(row):
    ...
    # Condition 1: Determine if the trial investigates targeted therapies or immunotherapies. Return 'Yes' if it does, otherwise return 'No'. Do not explain your answer. (equal_to Yes)
    fields_cond_0 = ["Interventions", "Summary", "Eligibility"]
    ...
    parsed_cond_0 = llm_parser(input_text_cond_0, "Determine if the trial investigates targeted therapies or immunotherapies. Return 'Yes' if it does, otherwise return 'No'. Do not explain your answer.").strip()
