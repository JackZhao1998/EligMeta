def parp_inhibitor_treatment_exclusion(trial: list[dict], target: list[dict]) -> float:
    ...
    for entry in trial:
        if entry.get('Entity', '').lower() in attended_entities:
            ...
            if text and text != 'NA':
                parsed = llm_parser(text, "Check if the patient has received previous treatment with PARP inhibitors. Return 'Yes' or 'No' only. Do not explain your answer.")
